...
import { View, Text, ActivityIndicator, RefreshControl, TouchableOpacity, TextInput, ScrollView, KeyboardAvoidingView, Platform, Alert, Share } from 'react-native';
...
const AdminReports = ({ navigation }) => {
  ...
  const generateMonthlyReportContent = () => {
    if (!monthlyReport) return '';
    
    const monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
    const monthName = monthNames[monthlyReport.month - 1];
    
    let content = `MRS EARTHMOVERS - MONTHLY REPORT\n`;
    content += `${monthName} ${monthlyReport.year}\n`;
    content += `Generated on: ${new Date().toLocaleString()}\n`;
    content += `Generated by: ${user.name}\n\n`;
    
    content += `========== SUMMARY ==========\n`;
    content += `Total Work Requests: ${monthlyReport.totalWorkRequests}\n`;
    content += `Completed Work: ${asCount(monthlyReport.completedWork)}\n`;
    content += `Vehicles Used: ${monthlyReport.totalVehicles}\n`;
    content += `Total Revenue: ₹${monthlyReport.totalRevenue}\n\n`;
    
    content += `========== REVENUE BY DAY ==========\n`;
    Object.entries(monthlyReport.revenueByDay || {}).forEach(([date, revenue]) => {
      content += `${new Date(date).toLocaleDateString()}: ₹${revenue}\n`;
    });
  ...
  const downloadMonthlyReport = async () => {
    if (!monthlyReport) {
      Alert.alert('Error', 'No monthly report data to download');
      return;
    }

    try {
      setDownloading(true);
      const reportContent = generateMonthlyReportContent();
      const monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
      const monthName = monthNames[monthlyReport.month - 1];
      const fileName = `MRS_Report_${monthName}_${monthlyReport.year}.txt`;
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        >
          <View style={styles.card}>
            ...
            {monthlyReport ? (
              <View style={styles.reportCard}>
                <View style={[styles.reportHeader, { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }]}>
                  <Text style={styles.reportTitle}>
                    {monthlyReport.year} - Month {monthlyReport.month}
                  </Text>
                  <TouchableOpacity
                    style={[styles.button, styles.buttonSecondary, { paddingHorizontal: 12, paddingVertical: 8 }]}
                    onPress={downloadMonthlyReport}
                    disabled={downloading}
                  >
                    <Text style={styles.buttonTextOnDark}>{downloading ? '⬇️ Download...' : '⬇️ Download'}</Text>
                  </TouchableOpacity>
                </View>
